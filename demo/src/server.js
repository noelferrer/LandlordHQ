...
app.post('/api/auth/demo', (req, res) => {
    let demoAdmin = db.get('admins').find({ username: 'demo_admin' }).value();
    ...
    if (!demoAdmin) {
        demoAdmin = {
            ...
            username: 'demo_admin',
            name: 'Demo Landlord',
            telegramId: process.env.OWNER_TELEGRAM_ID // Fallback to owner for initial setup
        };
        db.get('admins').push(demoAdmin).write();
    }

    const sessionToken = uuidv4();
    const sessionExpires = addMinutes(new Date(), 60 * 24).toISOString();
...
app.get('/api/dashboard', authenticateAdmin, (req, res) => {
    ...
    // Ensure we have the latest data from the bot process
    db.read();
    
    const stats = {
        totalTenants: db.get('tenants').filter({ adminId }).size().value(),
        activeTickets: db.get('tickets').filter({ adminId, status: 'open' }).size().value(),
        pendingPayments: db.get('payments').filter({ adminId, status: 'pending' }).size().value(),
        totalProperties: db.get('properties').filter({ adminId }).size().value(),
    };
    res.json(stats);
});
