        ...
        async function refreshDashboard() {
            try {
                ...
                if (recentBody) {
                    // Show both verified (manual) and pending (bot receipts)
                    const displayPayments = payments.slice().sort((a,b) => new Date(b.timestamp) - new Date(a.timestamp));
                            ...
                            <table style="width: 100%; border-collapse: collapse; text-align: left; font-size: 0.9rem;">
                                <thead>
                                    <tr style="border-bottom: 1px solid var(--border); color: var(--text-muted);">
                                        ...
                                        <th style="padding-bottom: 15px; font-weight: 600;">Proof</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ${displayPayments.slice(0, 3).map(p => {
                                        ...
                                        const statusColor = p.status === 'verified' ? 'var(--success)' : 'var(--warning)';
                                        const statusLabel = p.status === 'verified' ? 'Verified' : 'Pending';
                                        const proofUrl = p.fileId ? `${API_URL}/media/${p.fileId}` : null;
                                        const proofHtml = proofUrl ? `<button class="btn-icon" onclick="openLightbox('${p.mediaType || 'photo'}', '${proofUrl}')" style="width:28px; height:28px;"><i class="fas fa-receipt"></i></button>` : '<span style="color:var(--text-muted);">-</span>';
                                        
                                        return `
                                        <tr style="border-bottom: 1px solid var(--border);">
                                            <td style="padding: 12px 0; font-weight: 600;">${esc(t.name)}</td>
                                            <td style="padding: 12px 0; color: var(--text-muted);">${esc(pName)}</td>
                                            <td style="padding: 12px 0; font-weight: 600; color: ${statusColor};">₱${(parseFloat(p.amount) || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })} <small style="font-size:0.7rem; display:block; opacity:0.7;">${statusLabel}</small></td>
                                            <td style="padding: 12px 0; color: var(--text-muted);"><i class="far fa-calendar-alt"></i> ${new Date(p.timestamp).toLocaleDateString()}</td>
                                            <td style="padding: 12px 0;">${proofHtml}</td>
                                        </tr>
                                        `;
                                    }).join('')}
                                </tbody>
                            ...
                            <table style="width: 100%; border-collapse: collapse; text-align: left; font-size: 0.9rem;">
                                <thead>
                                    <tr style="border-bottom: 1px solid var(--border); color: var(--text-muted);">
                                        ...
                                        <th style="padding-bottom: 15px; font-weight: 600;">Date</th>
                                        <th style="padding-bottom: 15px; font-weight: 600;">Evidence</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ${pendingTickets.slice(0, 5).map(tk => {
                                        const hasEvidence = (tk.media && tk.media.length > 0) || tk.fileId;
                                        const evHtml = hasEvidence ? `<i class="fas fa-paperclip" style="color:var(--primary); cursor:pointer;" onclick="document.querySelector('.nav-item[onclick*=\\'support-section\\']').click();"></i>` : '-';
                                        return `
                                        <tr style="border-bottom: 1px solid var(--border);">
                                            ...
                                            <td style="padding: 12px 0; text-align:center;">${evHtml}</td>
                                        </tr>
                                        `;
                                    }).join('')}
                                </tbody>
